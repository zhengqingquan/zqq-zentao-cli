...
| GET,POST | `/products/:id/stories` | 需求列表/创建 | `stories` | `stories --product` |
...
| GET,PUT,DELETE | `/stories/:id` | 需求详情/更新/删除 | `story` | `story <id>` |
| POST | `/stories/:id/change` | 变更需求 | `storyChange` |  |
| POST | `/stories/:id/close` | 关闭需求 | `storyClose` |  |
| POST | `/stories/:id/active` | 激活需求 | `storyActive` |  |
| POST | `/stories/:id/assign` | 指派需求 | `storyAssignto` |  |
| GET,POST | `/stories/:id/estimate` | 需求估时 | `storyRecordEstimate` |  |
| DELETE | `/stories/:id/recall` | 撤销评审 | `storyRecall` |  |
| POST | `/stories/:id/review` | 评审需求 | `storyReview` |  |
| POST | `/stories/:id/submitreview` | 提交评审 | `storySubmitReview` |  |
...
统计：路由 **142** 条；本工具 REST 只读已覆盖文档选区中有 entry 的 GET（注册表约 **34** 个资源 / **48** 条新子命令，另含既有 tasks/users/projects 等）。写操作与缺 entry 路由未接。
