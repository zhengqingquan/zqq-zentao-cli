...
| GET/PUT/DELETE/POST 动作 | `/tasks/:id`… | `task` 详情与写（见完整清单） |
...
| Method | Path | 说明 | Entry | 本项目 |
|--------|------|------|-------|--------|
| GET,POST | `/tasks` | 任务列表/创建 | `tasks` | `tasks` / `my-tasks`；`task create` 用执行下 POST |
| GET,POST | `/executions/:id/tasks` | 任务列表/创建 | `tasks` | `tasks -e`；`task create --execution` |
| GET,PUT,DELETE | `/tasks/:id` | 任务详情/更新/删除 | `task` | `task <id>`；`task update\|delete <id>` |
| POST | `/executions/:id/tasks/batchCreate` | 批量创建任务 | `taskBatchCreate` |  |
| POST | `/tasks/batchCreate` | 批量创建任务 | `taskBatchCreate` |  |
| POST | `/tasks/:id/assignto` | 指派任务 | `taskAssignTo` | `task assign <id>` |
| POST | `/tasks/:id/start` | 开始任务 | `taskStart` | `task start <id>` |
| POST | `/tasks/:id/pause` | 暂停任务 | `taskPause` |  |
| POST | `/tasks/:id/restart` | 继续任务 | `taskRestart` |  |
| POST | `/tasks/:id/finish` | 完成任务 | `taskFinish` | `task finish <id>` |
| POST | `/tasks/:id/close` | 关闭任务 | `taskClose` | `task close <id>` |
| GET,POST | `/tasks/:id/estimate` | 任务记工时 | `taskRecordEstimate` |  |
| POST | `/tasks/:id/active` | 激活任务 | `taskActive` | `task activate <id>` |
...
| GET,POST | `/products/:id/bugs` | Bug 列表/创建 | `bugs` | `bugs --product`；`bug create --product` |
| GET | `/projects/:id/bugs` | 项目下 Bug | `projectBugs` | `bugs --project` |
| GET | `/executions/:id/bugs` | 执行下 Bug | `executionBugs` | `bugs --execution` |
| GET,PUT,DELETE | `/bugs/:id` | Bug 详情/更新/删除 | `bug` | `bug <id>`；`bug update\|delete <id>` |
| POST | `/bugs/:id/close` | 关闭 Bug | `bugClose` | `bug close <id>` |
| POST | `/bugs/:id/assign` | 指派 Bug | `bugAssign` | `bug assign <id>` |
| POST | `/bugs/:id/confirm` | 确认 Bug | `bugConfirm` | `bug confirm <id>` |
| POST | `/bugs/:id/resolve` | 解决 Bug | `bugResolve` | `bug resolve <id>` |
| POST | `/bugs/:id/active` | 激活 Bug | `bugActive` | `bug activate <id>` |
...
| `config/apiv2.php` | v2 路由（另一套；全文见 [zentao-rest-apiv2.md](./zentao-rest-apiv2.md)；本工具未用） |
...
统计：路由 **142** 条（与 `config/apiv1.php` 逐条对齐，2026-07-23 核对：路径 142/142，有 entry 的 Methods 与 `api/v1/entries/*.php` 一致）。缺 entry **7** 条见上节。本工具 REST 只读由注册表驱动；`bug`/`task`/`story` 写已接；其余写与缺 entry 路由未接。APIv2 不在本文范围。

核对脚本备忘：对 `apiv1.php` 的 `$routes[...]` 与本文「完整路由清单」路径集合做差集；对 entry 文件扫描 `function get|post|put|delete` 与 Method 列比对。
